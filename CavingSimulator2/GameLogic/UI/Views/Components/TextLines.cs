...
namespace CavingSimulator2.GameLogic.UI.Views.Components
{
    public class TextLines : View
    {
        ...
        public float PixelHeight { get { return LinesCount * LetterHeight; } }

        public void AddLine(string str, Color4 color)
        {
            GetCordsFromLowerPositionWidthHeight(LowerUpperPosition - Vector2.UnitY * LetterHeight * (lines.Count + 1), new Vector2(LetterWidth, LetterHeight), out Vector2 lPosition, out Vector2 uPosition);
            lines.Add(new UITextMesh(str, lPosition, uPosition, color, 1f));
        }
        public void UpdateLine(int line, string str, Color4 color)
        {
            RemoveLine(line);
            //if (lines[line] is not null) lines[line].Dispose();
            GetCordsFromLowerPositionWidthHeight(LowerUpperPosition - Vector2.UnitY * LetterHeight * (line + 1), new Vector2(LetterWidth, LetterHeight), out Vector2 lPosition, out Vector2 uPosition);
